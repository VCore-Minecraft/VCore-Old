...
import de.verdox.vcorepaper.utils.keys.LocationKey;
import de.verdox.vcorepaper.utils.keys.SplitChunkKey;
...
public class NBTLocation extends NBTHolderImpl<Location, NBTCompound> {
    ...
    protected NBTCompound getOrLoadLocationCompound() {
        if (Objects.isNull(this.nbtFile)) {
            ...
            this.nbtFile = vBlock.getCustomDataManager().getVCoreNBTModule().getLocationNBTFileStorage().getWorldStorage(worldChunk.worldName).getNBTFile(WorldChunk.getRegionX(worldChunk.x), WorldChunk.getRegionZ(worldChunk.z));
