...
public class SerializablePlayerInventory extends SerializableInventory {
    ...
    public SerializablePlayerInventory(@NotNull String id, @NotNull GameMode gameMode, @NotNull ItemStack[] storageContents, @NotNull ItemStack[] armorContents, @NotNull ItemStack[] enderChest, @Nullable ItemStack offHand, double health, int foodLevel, int level, float exp, @NotNull Set<PotionEffect> potionEffects) {
        ...
        List<String> serializedPotionEffects = potionEffects.stream().map(BukkitPlayerUtil::serializePotionEffect).collect(Collectors.toList());
    ...
    public boolean restoreInventory(@NotNull Player player, @Nullable Runnable callback) {
        BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eLade Rüstung&7...");

        ItemStack[] armorContents = deSerializeArmorContents();
        if (armorContents == null) {
            BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.CHAT, "&cKonnte Rüstung nicht wiederherstellen");
        } else {
            BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eRüstung wurde geladen");
            player.getInventory().setArmorContents(armorContents);
        }

        BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eLade Items&7...");
        ItemStack[] storageContents = deSerializeStorageContents();

        if (storageContents == null) {
            BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.CHAT, "&cKonnte Items nicht wiederherstellen");
        } else {
            BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eItems wurden geladen");
            player.getInventory().setStorageContents(storageContents);
        }

        BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eLade EnderChest&7...");
        ItemStack[] enderChest = deSerializeEnderChest();
        if (enderChest == null) {
            BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.CHAT, "&cKonnte EnderChest nicht wiederherstellen");
        } else {
            BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eEnderChest wurde geladen");
            player.getEnderChest().setContents(enderChest);
        }

        BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eLade Offhand&7...");
        BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&eOffhand wurden geladen");
        ...
        if (!serializedEffects.isEmpty()) {
            new ListBsonReference<String>(data, "potionEffects").getValue().stream()
                    .map(BukkitPlayerUtil::deSerializePotionEffect)
        ...
        BukkitPlayerUtil.sendPlayerMessage(player, ChatMessageType.ACTION_BAR, "&aSpielerdaten wurden geladen");
        if (callback != null)
            callback.run();
        return true;
    }
